...
class Gallery extends Component {
  ...
  filterImg = () => {
    ...
    const filteredImgDetails = imagesList.filter(
      each => each.id === activeImageId,
    )
    return filteredImgDetails
  }

  render() {
    const {activeImageId} = this.state
    const filteredImgDetails = this.filterImg()

    return (
      <div className="bg-container">
        {filteredImgDetails.map(eachItem => (
          <img
            key={eachItem.id}
            className="image"
